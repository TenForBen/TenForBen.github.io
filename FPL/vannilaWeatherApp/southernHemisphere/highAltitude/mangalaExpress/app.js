// Mangala Lakshadweep Express (12617/12618, Hazrat Nizamuddin <-> Ernakulam)
// — every stop on the route with its current weather, same idea as
// ../app.js (High Altitude Cities) with City/Country renamed
// Station/State (every row here is India, so "Country" would just say
// "India" 39 times) and the default sort flipped to route order instead
// of a ranking, since the point of this page is the journey, not a
// leaderboard.
//
...
// hourly scheduled workflow) or, once Refresh has been used in this
// browser, a live client-side re-fetch cached in localStorage. Local Time
// is the one column that's genuinely live, same trick ../app.js uses.
//
// Rows are keyed by `order` (route position), not `iso2` — every stop is
// India, and several share a state (Kerala alone has 13), so neither
// uniquely identifies a row here.
//
// The Koyilandi watch: stops.json's one entry with a `watch` field
// (`{ aboveC: 29 }`) gets its Temp cell highlighted whenever a reading
// meets or beats that threshold — see the README's "Koyilandi watch"
...
const LAST_RUN_KEY = "mangalaExpressWeather_lastRun";
const PREVIOUS_RUN_KEY = "mangalaExpressWeather_previousRun";
...
let compareOn = false;
let sortKey = "order";
let sortDir = 1; // route order, Delhi -> Ernakulam — the point of this page is the journey
...
// True once a stop's own watch threshold (stops.json's `watch.aboveC`) is
// met or beaten by its current reading — only Koyilandi has one, but this
// stays generic in case another stop ever gets a watch of its own.
function watchHit(row) {
  return !!(row.watch && row.weather && row.weather.temp >= row.watch.aboveC);
}
...
function weatherCells(row) {
  ...
  if (row.weather) {
    ...
    const watchClass = watchHit(row) ? " me-watch-hit" : "";
    tempCell = `<span class="${watchClass}" title="${escapeHtml(row.weather.description)}">${tempText}${
      watchHit(row) ? ` <span class="me-watch-badge" title="At or above the ${row.watch.aboveC}&deg;C watch threshold">&#9888;</span>` : ""
    }</span>`;
...
function renderTableRow(row) {
  ...
  return `
    <tr class="${watchHit(row) ? "me-watch-row" : ""}">
...
function renderCard(row) {
  ...
  return `
    <div class="ha-card${isOpen ? " ha-card-open" : ""}${watchHit(row) ? " me-watch-row" : ""}" data-order="${row.order}">
...
function renderTable() {
  ...
  const tbody = document.getElementById("meBody");
  tbody.innerHTML = rows.map(renderTableRow).join("");

  const cardsWrap = document.getElementById("meCards");
  ...
  document.getElementById("meCount").textContent = `${rows.length} stops`;
...
function renderUpdatedNote() {
  document.getElementById("meUpdated").textContent = current.generatedAt
...
const compareBtn = document.getElementById("meCompare");
...
function renderError() {
  ...
  document.getElementById("meCards").innerHTML =
