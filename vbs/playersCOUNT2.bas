Attribute VB_Name = "playersCOUNT2"
Sub playerCountAnalysisFPL6()

    Cells(34, 18).Value = "PLAYERS"
    ss = ActiveSheet.Name
    Set sht = ThisWorkbook.Worksheets(ss)
    LastRow18 = Cells(sht.Rows.Count, 18).End(xlUp).Row
     Cells(34, 19).Value = "PLAYERS_COUNT"
     Cells(34, 20).Value = "CAPTAIN_COUNT"
     Cells(34, 21).Value = "CAPTAINED_MGR"
     
    For r = 2 To 33 ' for all the rows of each manager
        For c = 20 To 34    ' for all the players of each row
            ...
            For t = 34 To LastRow18 ' comparing with already present player
